...
def main(cpi_path_pattern, wages_filepath):
    ...
    '''   
        # Identify highest average month-to-month change
        try:
            # We can use groupby on the *index* (level=1) of mtm_changes, which is 'Item'
            grouped_max = mtm_changes.groupby(level='Item')
            print(grouped_max.idxmax())
            print(grouped_max.max())
        # Catch any errors in case of unexpected data    
        except Exception as e:
            print(f"Could not determine max values: {e}")
        print("\n")
    '''
